(function(){
	var cx = document.forms.namedItem('assignuser');
	var jq = document.forms.namedItem('removeuser');

	var ev = document.querySelector('data#store_id');
	var gt = document.querySelector('ul#lastuserul');
	...
	var o = `/store/${ev.value}/removeuser/${ir.value}`;
	q.open('POST', o, true);
	...
	var o = `/store/${ev.value}/assignuser/${fu.value}`;
	p.open('POST', o, true);
