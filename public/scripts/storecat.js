...
function sx (f, l){
	...
	p.onreadystatechange	=	()=>{
		if(p.readyState === XMLHttpRequest.DONE){
			 var u = p.responseText;
			return l.setAttribute("value", u);
		}
	}
	var o = '/admin/c8/store/category';
	p.open('POST', o, true);
...
function lx (f){
	...
	var o = '/admin/c8/store/category/activate';
	p.open('POST', o, true);
	p.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
...
function rx (f){
	...
	var o = '/admin/c8/store/category/deactivate';
	p.open('POST', o, true);
	p.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
	p.send('selectedcat='+ encodeURIComponent(f));
}
